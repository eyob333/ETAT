...
import { useUpdateProjectDataMutation } from '../../redux/project/projectApiSlice';
import { projectSelector, userSelector } from '../../redux/store';
import { fetchProject, updateProjectState } from '../../redux/project/projectSlice';
...
const validationSchema = Yup.object().shape({
  ...
  projectStatus: Yup.string()
    .required('Required')
    .oneOf(['Ongoing', 'Completed'], 'Invalid status Type'),
...
export default function ProjectUpdateForm() {
  ...
  const [updateProject, { isLoading: loading }] = useUpdateProjectDataMutation();
  ...
  const onSubmit = async (values, { setSubmitting, setErrors }) => {
    console.log(new Date(filteredProject[0].start_date).toISOString().split('T')[0]);
    ...
    console.log(`${formattedEndDate}dd`);
    ...
    const status = projectStatus === 'Ongoing';
    ...
    formData.append('id', filteredProject[0].id);

    try {
      const res = await updateProject(formData).unwrap();
      console.log(`${res}jj`);
      dispatch(updateProjectState(res.project));
      setSubmitting(false);
  ...
  useEffect(() => {
    if (projects.length > 0) {
      setProjectStatus(filteredProject[0].status ? 'Ongoing' : 'Completed');
      setProjectArea(filteredProject[0].area);
    }
  }, [projects.length]);
  ...
  if (isLoading) {
    ...
    filteredProject = projects.filter((project) => project.id === parseInt(id));
    content = (
      <div className="">
        <center>
          ...
          <div className="text-center  px-8  w-full">
            <h1 className="block text-3xl font-bold text-gray-800 w-5/6">Update Project</h1>
          </div>
          <Formik
            initialValues={{
              projectTitle: filteredProject[0].title,
              projectDescription: filteredProject[0].body,
              projectStatus: filteredProject[0].status ? 'Ongoing' : 'Completed',
              projectArea: filteredProject[0].area,
              projectStartDate: new Date(filteredProject[0].start_date).toISOString().split('T')[0],
              projectEndDate: (filteredProject[0].end_date) ? new Date(filteredProject[0].end_date).toISOString().split('T')[0] : '',
            }}
