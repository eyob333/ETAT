...
import { fetchTraining, removeTraining } from '../../redux/training/trainingSlice';
import { trainingSelector } from '../../redux/store';
import { useDeleteTrainingMutation } from '../../redux/training/trainingApiSlice';
...
export default function Trainings() {
  ...
  const [deleteTraining, { isLoading: loading }] = useDeleteTrainingMutation();
  const { trainings, isLoading } = useSelector(trainingSelector);
  const [searchInput, setSearchInput] = useState('');

  const handleSearchInput = (training) => {
    setSearchInput(training.target.value);
  };
  ...
  const handleDelete = async (id) => {
    try {
      console.log(id);
      const res = await deleteTraining(id).unwrap();
      setOpen(null);
      if (res.message === 'Training deleted successfully') {
        dispatch(removeTraining(id));
        toast.success('you have successfully deleted a Training');
      }
    } catch (error) {
      if (error.status === 400) {
  ...
  if (isLoading) {
    ...
    filteredTraining = trainings.filter((training) => training.title.toLowerCase().includes(searchInput.toLowerCase()));
    content = (
      <section className="lg:pt-[30px]  pt-6 pb-12 lg:pb-[0px]">
        <div className="container">
          <div className="flex flex-wrap px-6 pb-6 gap-x-6 gap-y-10 md:gap-0 -mx-2">
            {filteredTraining.length === 0 ? <h1 className="text-2xl font-semibold text-center">No Training Found</h1>
                  ...
                  >
                    <div className="absolute top-2 right-2 text-lg" type="button">
                      <div className="flex gap-2">
                        <Link className="text-green-300 text-xl" to={`/admin/trainingDetail/${item.id}`} title="View Details">
                          <MdReadMore />
                        </Link>
                        <Link className="text-blue-300" to={`/admin/updateTraining/${item.id}`} title="Edit Training">
                          <AiFillEdit />
                        </Link>
                        <button className="text-red-600" type="button" onClick={() => handleOpenModal(item.id)} title="Delete Training">
                          <AiFillDelete />
                        </button>
      ...
      <div className="flex justify-between items-center px-8">
        <div>
          <div className="mt-1 relative lg:w-64">
            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-trainings-none">
        ...
        <Link to="/admin/addTraining" className="px-12 pb-2 pt-1 rounded-md bg-mainColor text-white lg:w-56 w-52 ml-3 mb-2">Add New Training</Link>
      </div>
      <div>
        {content}
      </div>
